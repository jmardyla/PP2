...
public:
    TwoDimensionMatrix();
    explicit TwoDimensionMatrix(const MatrixElement matrix[2][2]);
    MatrixElement get(MatrixElement row, MatrixElement column) const;
    static constexpr size_t size() { return size_; }
    TwoDimensionMatrix& operator=(TwoDimensionMatrix);
    MatrixElement* operator[](size_t position);
    const MatrixElement* operator[](size_t position) const;
    friend std::ostream& operator<<(std::ostream &os, TwoDimensionMatrix);
    friend std::istream& operator>>(std::istream &is, TwoDimensionMatrix&);
