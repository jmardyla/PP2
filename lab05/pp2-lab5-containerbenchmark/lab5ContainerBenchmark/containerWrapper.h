...
public:
    ...
    DequeWrapper(const value_type* array, std::size_t size);

    void push_back(const value_type& element) override;
    void push_front(const value_type& element) override;

    void insert(const value_type& element, std::size_t pos) override;

    std::size_t size() const override { return impl_.size(); }

    value_type& at(std::size_t pos) override;
    ...
    std::size_t find(const value_type& key) const override;
