...
class Linux
{
    ...
    inline static std::size_t open_source_sympathizers_ = 0;
...
public:
    ...
    explicit Linux(const std::string& distro) {
        distribution_ = distro;
        std::cout << "+" << distribution_ << '\n';
        open_source_sympathizers_++;
    }
    ~Linux() {
    ...
    int add_user(const std::string& user_name, const std::string& password);
    User user(int user_id);
    std::string user_home_directory(int user_id);
    std::optional<std::string> graphic_environment() const;
    void set_graphic_environment(std::optional<std::string> graph_env);
};
